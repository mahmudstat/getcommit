...
test_that("plot_commits works for both clock and line types", {
  comm <- tibble::tibble(
  ...
  # Line plot
  p_line <- plot_commits(comm, type = "line")
  expect_s3_class(p_line, "ggplot")
  layer_types <- sapply(p_line$layers, function(x) class(x$geom)[1])
  expect_true(any(layer_types == "GeomBar"))
  expect_equal(p_line$labels$y, "Number of commits")
  expect_equal(p_line$labels$fill, "Author")
